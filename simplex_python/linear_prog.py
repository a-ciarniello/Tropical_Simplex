...
if __name__ == "__main__":
    ...
    # Definisce un semplice LP: min x + y s.t. x >= 2, y >= 3
    # In algebra tropicale, per fare sì che x >= 2, usiamo un valore negativo per la variabile
    objective = [((ColKind.VAR, 0), Sign.POS, 0), ((ColKind.VAR, 1), Sign.POS, 0)]
    ineqs = [
        [((ColKind.VAR, 0), Sign.POS, 0), ((ColKind.AFFINE, None), Sign.NEG, 5)],  # x vs 5 
        [((ColKind.VAR, 1), Sign.POS, 0), ((ColKind.AFFINE, None), Sign.NEG, 5)]   # y vs 5
    ]
    
    var_names = lambda i: f"x{i}"
    
    lp = lp_mod.init(var_names, 2, objective, ineqs)
    
    print("LP creato con successo!")
    print(f"Dimensioni: {lp.dim()}, Disequazioni: {lp.nb_ineq()}")
    
    # Stampa la struttura dell'LP
    lp.pretty_print()
    
    # Test punto feasible: scegliamo valori più piccoli delle costanti per far "vincere" le variabili
    point = np.array([1.0, 2.0])  # x=1, y=2 (più piccoli di 5)
    print(f"\nTesting point {point}:")
    for i in range(lp.nb_ineq()):
        print(f"  Inequality {i} raw row: {lp.get_row((RowKind.INEQ, i))}")
        # Debug: calcoliamo manualmente i valori
        row = lp.get_row((RowKind.INEQ, i))
        for col_index, sign, entry in row:
            val = lp.compute_entry_plus_var(entry, col_index, point)
            print(f"    {col_index}, {sign}, entry={entry} -> val={val}")
        
        arg = lp.compute_slack_args((RowKind.INEQ, i), point)
        print(f"  Inequality {i} argmin: {arg}")
    print(f"Punto {point} feasibile: {lp.is_point_feasible(point)}")
    
    point_bad = np.array([6.0, 7.0])  # x=6, y=7 (più grandi di 5, dovrebbero far "vincere" le costanti)
    print(f"\nTesting point {point_bad}:")
    for i in range(lp.nb_ineq()):
        arg = lp.compute_slack_args((RowKind.INEQ, i), point_bad)
        print(f"  Inequality {i}: {arg}")
    print(f"Punto {point_bad} feasible: {lp.is_point_feasible(point_bad)}")
